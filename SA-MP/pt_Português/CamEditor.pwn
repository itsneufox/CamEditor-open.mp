...
public OnPlayerSpawn(playerid)
{
    ...
    {
        SendClientMessage(playerid, -1, "{FFFFFF}Digite {8EFF8E}/editorcamera{FFFFFF} para acessar o editor de c�mera");
    }
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    if (!strcmp(cmdtext, "/fechareditorcamera", true))
    {
        if (IsCreating[playerid])
        {
            ...
            coordInfo[playerid][MoveSpeed]      = 1000;
            coordInfo[playerid][RotSpeed]       = 1000;
            SendClientMessage(playerid, -1, "{8EFF8E}Editor de C�mera{FFFFFF} foi fechado");
        }
        else 
        {
            SendClientMessage(playerid, -1, "{F58282}Erro: {FFFFFF}O Editor de C�mera n�o est� ativo no momento");
        ...
        if (IsCreating[playerid] == true)
        {
            if (SettingFirstLoc[playerid] == true)
            {
                ...
                if (IsReSettingStart[playerid] == true)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_MENU, DIALOG_STYLE_LIST,"{FFFFFF}Editor de C�mera - Pr�ximo Passo?","{FFFFFF}Visualizar Movimento da C�mera\nModificar Ponto Inicial\nModificar Ponto Final\nAjustar Velocidades\nSalvar em Arquivo","{8EFF8E}Confirmar","{F58282}Cancelar");
                    IsReSettingStart[playerid]      = false;
                    IsReSettingEnd[playerid]        = false;
                    SettingFirstLoc[playerid]       = false;
                    SettingLastLoc[playerid]        = false;
            ...
            else if (SettingLastLoc[playerid] == true)
            {
                ...
                format(string, sizeof(string), "{FFFFFF}Por favor, insira a dura��o desejada para o {F58282}movimento{FFFFFF} em milissegundos\n\nVelocidade atual do movimento: \t{F58282}%i milissegundos\n{FFFFFF}Velocidade atual da rota��o: \t{F58282}%i milissegundos\n\n\n{CFCFCF}Nota: {FFFFFF}1 segundo = 1000 milissegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                ShowPlayerDialog(playerid, DIALOG_MOVE_SPEED, DIALOG_STYLE_INPUT, "{FFFFFF}Configura��es de Dura��o do Movimento", string,"{8EFF8E}Confirmar","{F58282}Cancelar");
                GetPlayerCameraPos(playerid, fPX, fPY, fPZ);
                GetPlayerCameraFrontVector(playerid, fVX, fVY, fVZ);
                ...
                if (IsReSettingEnd[playerid] == true)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_MENU, DIALOG_STYLE_LIST,"{FFFFFF}Editor de C�mera - Pr�ximo Passo?","{FFFFFF}Visualizar\nAlterar In�cio\nAlterar Fim\nAlterar Velocidade\nSalvar","{8EFF8E}Confirmar","{F58282}Cancelar");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case DIALOG_MENU:
        {
                ...
                switch(listitem)
                {
                    ...
                    case 3: //Alterar velocidade
                    {
                        new 
                            string[512]
                        ;
                        format(string, sizeof(string), "{FFFFFF}Por favor, insira a dura��o desejada para o {F58282}movimento da c�mera{FFFFFF} em milissegundos\n\nVelocidade atual do movimento: \t{F58282}%i milissegundos\n{FFFFFF}Velocidade atual da rota��o: \t{F58282}%i milissegundos\n\n\n{F58282}Nota: {FFFFFF}1 segundo = 1000 milissegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                    ...
                    case 4: //Exportar
                    {
                        ShowPlayerDialog(playerid, DIALOG_EXPORTNAME, DIALOG_STYLE_INPUT, "{FFFFFF}Salvar Movimento da C�mera","{FFFFFF}Digite um nome descritivo para este movimento de c�mera","{8EFF8E}Salvar","{FFCC00}Voltar");
                    }
                }
            }
            else
            {
                CancelFlyMode(playerid);
                SendClientMessage(playerid, -1, "{F58282}Voc� saiu do editor de movimento de c�mera.");
            ...
            if (response)
            {
                if (strlen(inputtext))
                {
                    if (IsNumeric(inputtext))
                    {
                        ...
                        format(string, sizeof(string), "{FFFFFF}Por favor, insira a dura��o desejada para a {F58282}rota��o{FFFFFF} em milissegundos\n\nVelocidade atual do movimento: \t{F58282}%i milissegundos\n{FFFFFF}Velocidade atual da rota��o: \t{F58282}%i milissegundos\n\n\n{F58282}Nota: {FFFFFF}1 segundo = 1000 milissegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                        ShowPlayerDialog(playerid, DIALOG_ROT_SPEED, DIALOG_STYLE_INPUT, "{FFFFFF}Velocidade da Rota��o", string,"{8EFF8E}Confirmar","{F58282}Cancelar");
                ...
                else
                {
                    ...
                    format(string, sizeof(string), "{FFFFFF}Por favor, insira a dura��o desejada para o {F58282}movimento{FFFFFF} em milissegundos\n\nVelocidade atual do movimento: \t{F58282}%i milissegundos\n{FFFFFF}Velocidade atual da rota��o: \t{F58282}%i milissegundos\n{FF0000}Voc� precisa inserir um valor\n\n{F58282}Nota: {FFFFFF}1 segundo = 1000 milissegundos", coordInfo[playerid][MoveSpeed], coordInfo[playerid][RotSpeed]);
                    ShowPlayerDialog(playerid, DIALOG_MOVE_SPEED, DIALOG_STYLE_INPUT, "{FFFFFF}Velocidade do Movimento", string,"{8EFF8E}Confirmar","{F58282}Cancelar");
                }
            }
            else
        ...
        case DIALOG_EXPORTNAME:
        {
            if (response)
            {
                ...
                {
                    ShowPlayerDialog(playerid, DIALOG_EXPORTNAME, DIALOG_STYLE_INPUT, "{FFFFFF}Salvar movimento","{FFFFFF}Digite um nome para este movimento de c�mera\n{F58282}Voc� precisa digitar um texto","{8EFF8E}Confirmar","{F58282}Cancelar");
                }
            }
            else
            {
...
public ExportMovement(playerid, inputtext[])
{
    ...
    format(myOutpString, sizeof(myOutpString), "{FFFFFF}Movimento da c�mera salvo como {F58282}%s{FFFFFF} na pasta scriptfiles!\n\nO que voc� gostaria de fazer agora?", filename);
...
stock FlyMode(playerid)
{
    ...
    SendClientMessage(playerid, -1, "{8EFF8E}Editor de C�mera{FFFFFF} foi ativado com sucesso");
    SendClientMessage(playerid, -1, "{FFFFFF}Use {F58282}~k~~GO_FORWARD~, ~k~~GO_BACK~, ~k~~GO_LEFT~ e ~k~~GO_RIGHT~{FFFFFF} para navegar no espa�o 3D");
